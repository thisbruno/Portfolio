...
<v-app class="black lighten-4 pt-5">
  ...
  <v-layout row wrap>
    ...
    <v-flex xs10 sm3 md3 offset-xs1 class="cyan--text headline">
      <v-card class="mb-2 mt-4 font2">
        <v-card-title class=" title justify-center primary--text text--darken-1 cyan lighten-3" >
          <span text-xs-center class="text-uppercase">Languages</span>          
        </v-card-title>        
         </v-card >
        <v-card class="mb-2" v-for="lang in langs" :key="lang.title" >
          <v-card-text class=" body-2 "  >
         <div class="subheading text-xs-center cyan--text text-uppercase underline" >{{lang.title}}</div>
            <div class="body-2 text-xs-center">{{lang.text}}</div>
          </v-card-text>
        </v-card>     
    </v-flex> 
  </v-layout> 
...
.font2{
  font-family: vidaloka, noto sans ;  
}
.color1{
 color: #E31B6D;
}
